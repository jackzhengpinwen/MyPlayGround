...
fun getBundleTaskOutput(project: Project, agpVersion: String, variantName: String): Provider<RegularFile> {
    ...
    val task = project.tasks.named(bundleTaskName, BundleLibraryClassesJar::class.java)

    val outputMethod = try {
        BundleLibraryClassesJar::class.java.getDeclaredMethod(getOutputPropertyName(agpVersion))
...
private fun getBundleTaskName(agpVersion: String, variantName: String) = when {
    ...
    else -> "bundleLibCompileToJar$variantName"
}
