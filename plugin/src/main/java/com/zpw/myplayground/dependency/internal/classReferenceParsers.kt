...
internal class JarReader(
    ...
    override fun parseBytecode(): Set<String> {
        return zipFile.entries().toList()
            ...
            .flatMap { classEntry ->
                zipFile.getInputStream(classEntry).use { BytecodeParser(it.readBytes()).parse() }
            }.toSet()
    }
}
...
internal class ClassSetReader(
    ...
    override fun parseBytecode(): Set<String> {
        return classes.flatMap { classFile ->
            classFile.inputStream().use { BytecodeParser(it.readBytes()).parse() }
        }.toSet()
    }
}

private class BytecodeParser(private val bytes: ByteArray) {
    /**
